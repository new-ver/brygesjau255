<script lang="ts">
	...
	function getPhaseDisplay(): string {
		...
		switch (gameState.status) {
			...
			case 'night':
				return `Night ${gameState.day_number}`;
			case 'day':
				return `Day ${gameState.day_number}`;
			case 'voting':
				return `Voting - Day ${gameState.day_number}`;
			case 'finished':
				return 'Game Over';
			default:
...
<div class="game-container">
	<header class="game-header">
		<button class="leave-btn" on:click={() => goto('/')}> ← Leave Game </button>
						...
						<div class="player-card-content">
							<div class="card-header">
								...
								<span
									...
									class:alive={currentPlayer.is_alive}
									class:dead={!currentPlayer.is_alive}
								>
							...
							<div
								...
								class:cult={currentPlayer.affiliation === 'cult'}
								class:townsfolk={currentPlayer.affiliation === 'townsfolk'}
							>
							...
							<div class="card-warning">⚠️ Keep this side private - do not show other players!</div>
						</div>
					</div>
							...
							<div class="game-stats">
								<div class="stat-card">
									...
									<span class="stat-value"
										>{allPlayers.filter((p) => p.is_alive).length}/{allPlayers.length}</span
									>
								</div>
								<div class="stat-card">
...
<style>
	...
	.flip-card-front {
		background:
			linear-gradient(135deg, rgba(139, 0, 0, 0.1), rgba(25, 25, 112, 0.1)),
